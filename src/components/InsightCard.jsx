...
export default function InsightCard({ item, readMoreLabel, comingSoonLabel = "Coming soon" }) {
    ...
    const cardClasses =
        "group rounded-2xl border border-slate-200 bg-white shadow-sm flex flex-col overflow-hidden transition-all duration-500 hover:shadow-premium hover:-translate-y-1";
    ...
    return (
        <Wrapper {...wrapperProps}>
            <div className="relative h-52 overflow-hidden">
                ...
                {isComingSoon && (
                    <span className="absolute top-4 right-4 text-[10px] font-bold uppercase tracking-widest px-3 py-1.5 rounded-full bg-slate-900/80 text-white shadow-sm">
                        {comingSoonLabel}
                    </span>
                )}
            </div>
            <div className="p-8 flex flex-col gap-4 flex-1">
                ...
                <div className="pt-4 text-xs font-bold uppercase tracking-widest text-slate-900 inline-flex items-center gap-2 group-hover:gap-3 transition-all group-hover:text-[#b78b45]">
                    {actionLabel}
                    {!isComingSoon && <ArrowRight className="h-3.5 w-3.5" />}
                </div>
            </div>
        </Wrapper>
    );
